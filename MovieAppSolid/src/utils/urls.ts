export const createApiUrl = (apiURL: string, searchParams?: Record<string, string | number | boolean>) => {
    const baseUrl = new URL(import.meta.env.PUBLIC_MOVIE_DB_API_URL);
    const url = new URL(`/3${apiURL}`, baseUrl);
    
    url.searchParams.append('api_key', import.meta.env.PUBLIC_MOVIE_DB_API_KEY);
...
export const createMovieDetailsUrl = (id: string | number) => createApiUrl(`/movie/${id}`);